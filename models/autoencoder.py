...
class AutoEncoder(Module):
    ...
    def __init__(self, config, encoder):
        ...
        self.diffusion = DiffusionTraj(
            net = self.diffnet(point_dim=18, context_dim=config.encoder_dim, tf_layer=config.tf_layer, residual=False), #TODO: Check dimension here
    ...
    def generate(self, batch, node_type, num_points, sample, bestof,flexibility=0.0, ret_traj=False, sampling="ddpm", step=100):
        ...
        #initial conditions must be changed so as to accomodate the center position
